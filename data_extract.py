        ...
        for each_link in links:
            ...
            if entry_key.startswith('home') or entry_key.startswith('vis'):
                if entry_key.startswith('home'):
                    entry_offset = 2 * NUM_PLAYERS * player_stats_number() #len(bs_keys)
                elif entry_key.startswith('vis'):
                    entry_offset = 2 * NUM_PLAYERS * player_stats_number() + team_stats_number() #len(ls_keys)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-test_mode", default=False, help ="")
